...
# TODO mejorar, para que verifique que los elementos dentro tambien tenga los tipos de datos correctos


def gaussSeidel_args_types_checking(fn):
    def wrapper(*args):
...
def gaussSeidel_args_transform_np_array(fn):
    def wrapper(*args):
        ...
        tempList = list(args)
        tempList = [np.array(arg, dtype=float) if isinstance(
